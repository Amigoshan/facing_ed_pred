...
def video_to_frames(filename=None):
    #returns 0-255 array of shape [frames, w,h, 3]
    if not filename:
        return []
    frames = []
    ...
    cap.release()
    #ipdb.set_trace() # check concat
    frames = np.asarray(frames)
    return frames

def frames_to_video(frames, filename, frame_rate=30.0):
    #ipdb.set_trace() # check frame size and frame whether in 255 or in 1
    frame_size = frames.shape[-3:]
    fout = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), frame_rate, (int(frame_size[1]), int(frame_size[0])))
    ...
    for i, frame in enumerate(frames):
        ...
        #ipdb.set_trace()
        fout.write(frame2.astype('uint8'))
    fout.release()
    return
...
def put_arrow_proper(img, dir, centerx=150, centery=96, new_img=True):
    # print type(img), img.dtype, img.shape
    if new_img:
        img = img.copy()
    cv2.line(img, (centerx-30,centery), (centerx+30,centery), (0, 255, 0), 2)
    cv2.line(img, (centerx,centery-30), (centerx,centery+30), (0, 255, 0), 2)

    cv2.arrowedLine(img, (centerx,centery), (int(centerx+40*dir[0]),int(centery-40*dir[1])), (0, 0, 255), 4)

    return img


def put_arrow(img, dir, centerx=150, centery=96, new_img=True):
    # print type(img), img.dtype, img.shape
    if new_img:
        img = img.copy()
...
#def put_arrow_video(model, filename, boxes):
...
def im_crop(image, maxscale=0.2, low_resol=False):
