...
exp_name = exp_prefix+'encoder_decoder_warp0.1_relu'
...
imgdataset = MnistManiDataset(warping = True)
...
while True:
	...
	for sample in dataloader:  # loop over the dataset multiple times
		...
		inputState = Variable(sample['wdata'].unsqueeze(1).cuda(),requires_grad=True)
